...
inline void solve(){
    ...
    vi od(n),nod(n);
    vi zero_indexes;
    zero_indexes.push_back(0);
    incr(i,0,n){
        if(data[i]==0) zero_indexes.push_back(i);
        else if(data[i]&1) nod[i] = 1;
        else od[i] = 1;
    }
    zero_indexes.push_back(n-1);
    prefix_sum <int> odd(od);
    prefix_sum <int> nodd(nod);
    ll ans = 0;
    incr(i,1,zero_indexes.size()-1){
        int leftod = odd.sum(zero_indexes[i-1],zero_indexes[i]);
        int rightod = odd.sum(zero_indexes[i],n-1);
        int leftnod = nodd.sum(zero_indexes[i-1],zero_indexes[i]);
        int rightnod = nodd.sum(zero_indexes[i],n-1);
        ans+=leftod*rightod;
        ans+=leftnod*rightnod;
    }
    cout << ans << nl;
}
signed main(){
