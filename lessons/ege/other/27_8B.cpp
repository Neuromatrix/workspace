...
inline void solve(){
    ...
    vector <int> mods(n);
    vector <int> notodd(n);
    vector <int> d13andN(n);
    vector <int> d13odd(n);
    incr(i,0,n){
        cin >> data[i];
        if(data[i]%13==0) mods[i] = 1;
        if(data[i]%2==1) notodd[i] = 1;
        if(data[i]%13==0 and data[i]%2==0) d13odd[i] = 1;
        if(data[i]%13==0 and data[i]%2==1) d13andN[i] = 1;
    }
    
    prefix_sum <int> div13(mods);
    prefix_sum <int> cntnodd(notodd);
    prefix_sum <int> d13n(d13andN);
    prefix_sum <int> d13od(d13odd);
    ll ans = 0;
    incr(i,0,n-5){
        int j = i+5;
        int k = n-1;
        if(data[i]%13==0 and data[i]%2==1){
            ans+=n-j-cntnodd.sum(j,n-1);
        } else if (data[i]%13==0 and data[i]%2==0){
            ans+=cntnodd.sum(j,n-1);
        } else if (data[i]%13!=0 and data[i]%2==1){
            ans+=d13od.sum(j,n-1);
        } else {
            ans+=d13n.sum(j,n-1);
        } 
