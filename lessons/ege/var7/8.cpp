...
inline void __with__permutation(){
    ...
    do
    {
        if(s[0]!='y' and s.find("eu")==string::npos) cnt++; // if 
            ...
            for (const auto & e3 : s) {
                for (const auto & e4 : s) {
                    for (const auto & e5 : s) {
                        ...
                        // if 
                        int y = count(tmp.begin(), tmp.end(),'y');
                        bool ff = (tmp[0]!='y');
                        bool ll = (tmp[4]!='y');
                        if(y>1) continue;
                        if(y==0)cnt.insert(tmp); 
                        else {
                            int ps = tmp.find('y');
                        bool place = true;
                        if(tmp[ps-1]=='e' or tmp[ps+1]=='e') place = false;

                        if(ff and ll and place) cnt.insert(tmp); 
                        }
