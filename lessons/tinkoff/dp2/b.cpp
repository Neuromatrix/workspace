...
void dfs0(ll node, ll par,vector<vector<pii> >& g,vector<ll>& dp, vector<ll>& size){
    dp[node] = 0;
    size[node] = 1;
    for (auto it : g[node]) {
        ll nebr = it.first;
...
void reroot(ll from, ll to,ll w,vector<ll>& dp,vector<ll>& size){
    dp[from] -= size[to]*w + dp[to];
    size[from] -= size[to];
    size[to] += size[from];
    dp[to] += size[from]*w + dp[from];
}
 
void dfs1(ll node, ll par, vector<vector<pii> >& g,vector<ll>& dp, vector<ll>& ans,vector<ll>& size){
    ans[node] = dp[node];
    for (auto it : g[node]) {
        ll nebr = it.first;
        ll w = it.second;
        if (par != nebr) {
            reroot(node, nebr,w, dp, size);
            dfs1(nebr, node, g, dp, ans,
                 size);
            reroot(nebr, node,w, dp, size);
        }
    }
}
vector<ll> pathSum(vector<vector<pii> > &g,
                   ll N){
    vector<ll> dp(N), ans(N), size(N);
    dfs0(0, -1, g, dp, size);
