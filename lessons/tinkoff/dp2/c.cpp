...
inline void solve(){
	...
	int ret = 0, best_i = -1;
	for (int i = 0; i < n; ++i) {
		int ll = upper_bound(A.begin(), A.end(), a[i] - m) - A.begin();
		int rr = lower_bound(A.begin(), A.end(), a[i] + m) - A.begin() + 1;
		dp[i] = max(F.query(ll), G.query(sz(A) + 1 - rr));
		dp[i].first += 1;
		int k = Index(a[i]);
		F.add(k, pii{dp[i].first, i});
		G.add(sz(A) + 1 - k, pii{dp[i].first, i});
		if (ret < dp[i].first) {
			ret = dp[i].first;
			best_i = i;
        }
