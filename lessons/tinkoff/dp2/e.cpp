...
int megametr(const vector <vector<int>> &G){//наутральная нумерация
    int root = 1;
    vi max_dist_f = bfs_distance_from(G,root).second;
    int max_ind_s = max_element(max_dist_f.begin(),max_dist_f.end())-max_dist_f.begin();
    vector <int> max_dist_s = bfs_distance_from(G,max_ind_s).second;
    int max_ind_t = max_element(max_dist_s.begin(),max_dist_s.end())-max_dist_s.begin();
    vector <int> max_dist_t = bfs_distance_from(G,max_ind_t).second;
    int mx = -INF;
    for(int i = 1; i < sz(G); i++){
        if(i==max_ind_t || i==max_ind_s) continue;
        mx = max(mx, max_dist_s[i]+max_dist_t[i]);
    }
    return mx+max_dist_s[max_ind_t];
}
inline void solve(){
    int n;
    cin >> n;
