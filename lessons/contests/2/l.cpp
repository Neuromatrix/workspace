...
template <typename T>
T binPow(T a, T pow,long long mod = 1e9+7){
    if (a == 1 || pow == 0) {
        return 1LL;
    } else if (pow == 1) {
        return a%=mod;
    } else {
        T  part = binPow(a, pow >> 1,mod)%mod;
        if (pow & 1) {
...
inline void solve(){
    ll n, q;
    cin >> n >> q;
    vector <vector<ld>> table(n+1,vector <ld>(q+1,0));
    for(int j = 1; j <=min(q,6ll); j++){
        table[1][j] = 1.0/6.0;
    }
    for(int i = 2; i <= n; i++){
        for(int j = 1; j <=q; j++){
