...
pii f(pii a){
    pii tmp;
    tmp.first = max(a.first,a.second);
    tmp.second = min(a.first,a.second);
    return tmp;
}
inline void solve(){
    int n;
    ...
    ll ans = 0;
    mn = INF;
     mn = min({abs(b[0]-a[0]),abs(b[n-1]-a[0]),abs(b[fs]-a[0])});
    if(mn==abs(b[0]-a[0])) used[0][2] = true;
    if (mn==abs(b[n-1]-a[0])) used[0][3] = true;
    ans+=mn;
    anss[0] = mn;
    mn = INF;
    mn = min({abs(b[0]-a[n-1]),abs(b[n-1]-a[n-1]),abs(b[fe]-a[n-1])});
    if(mn==abs(b[n-1]-a[n-1])) used[1][3] = true;
    if (mn==abs(b[0]-a[n-1])) used[1][2] = true;
    ans+=mn;
    anss[1] = mn;
    mn = INF;

     mn = min({abs(b[0]-a[0]),abs(a[n-1]-b[0]),abs(a[ss]-b[0])});
    ans+=mn;
    mn = INF;
    anss[2] = mn;
     mn = min({abs(b[n-1]-a[0]),abs(b[n-1]-a[n-1]),abs(a[se]-b[n-1])});
    ans+=mn;
    mn = INF;
    anss[3] = mn;
    cout << ans << nl;
