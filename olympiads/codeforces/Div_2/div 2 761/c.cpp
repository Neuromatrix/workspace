...
inline void solve(){
    ...
    int iter = 0;
    set <int> not_used;
    incr(i,1,n+1) not_used.insert(i);
    vi next;
    fca(it,data){
        if(not_used.find(it)!=not_used.end()) not_used.erase(it);
        else next.pb(it);
    }
    sort(all(next));
    fca(it,not_used){
        if(next[iter]%(next[iter]-it)!=it){bad= true; continue;}
        iter++;
    }
    if(bad) cout << -1 << nl;
    else cout << not_used.size() << nl;
}
signed main(){
    IOS;
    // prepare();
