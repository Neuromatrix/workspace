...
bool isExcape(int x, int timer)
{
  if(!isValid(x,timer)) return false;
...
inline void solve(){
    ...
    incr(i,1,n+1) cout << g[i] << endl;
    preprocess_lava_flow();
    // if(bfs_escape()) cout << "YES" << nl;
    // else cout << "NO" << nl;
}
signed main(){
    IOS;
