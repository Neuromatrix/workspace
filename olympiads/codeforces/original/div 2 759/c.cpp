...
inline void solve(){
    ...
    sort(all(plus));
    bool bad = true;
    if(!sz(plus)) bad = false;
    if(sz(minn)>0 and sz(plus)>0 and
     minn.back()>plus.back()) bad = false;
    ll now = -1;
    if(sz(minn))
        now = sz(minn)-1;
        while(now>=k-1){
            if(now==sz(minn)-1 && !bad){
                res+= minn[now];
            } else {
                res+=minn[now]*2;
            }
