...
int check(string s, int c, int i){
    int l = max(i-2,0);
    int r = min(i+2,sz(s)-1);
    string subd = s.substr(l,r-l+1);
    int pr = countSubstring(subd,"abc");
    s[i] = c;
    subd = s.substr(l,r-l+1);
    int ne =  countSubstring(subd,"abc");
    return ne-pr;
}
inline void solve(){
    int n, q;
    ...
    string data;
    cin >> data;
    int now =  countSubstring(data,"abc");
    while (q--){
        int i; char c;
        cin >> i >> c;
        now+=check(data,c,i-1);
        data[i-1] = c;
        cout << now << endl;
    }
