...
pii combine(pii a, pii b){
    pii x = {max(a.first,b.first),min(a.second,b.second)};
    if(x.first>x.second) return {-1,-1};
    else return x;
}
template <typename T>
class lseg_tree
{
    private:
        ...
        void build (const vector <pii> &a, int v, int tl, int tr) {
            if (tl == tr) {
                tree[v] = a[tl];
            } else {
                int tm = (tl + tr) / 2;
    ...
    public:
        ...
        T query(int l, int r){
            return query(1,0,size_it-1,l,r);
        }
        lseg_tree(vector <pii> &a){
            init(a);
        }
        lseg_tree(){}
        void print(){
...
inline void solve(){
    ...
    lseg_tree <pii> h;
    h.init(data);
    // cout << combine(combine(data[0],data[1]),data[2]) << nl;
    ll ans = 0;
    incr(i,0,n){
        incr(j,i,n){
            pii inter = h.query(i,j);
            // cout << inter << nl;
            if(inter.second-inter.first<=m2 and inter.second-inter.first>=m1) ans++;
        }
    }
    cout << ans << nl;
}
