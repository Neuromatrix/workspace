...
inline void solve(){
    ...
    seev(rails,k);
    vvi dp(k+1,vi(n+1));
    int mx = *max_element(all(rails))+1;
    incr(i,0,n+1) dp[0][i] = mx;
    incr(i,0,k+1) dp[i][0] = 0;
    ...
    incr(i,1,k+1){
        incr(j,1,n+1){
            if (j < rails[i]){
                ...
                dp[i][j]=min(dp[i-1][j],dp[i-1][j-rails[i]]+1);
            }
        }
    }
    int ans = dp[k][n];
    if(ans==mx) cout << "NO" << nl;
    else{
        cout << ans << nl;
        set <int> ann;
        int p = n;
