...
ll binPow(ll a, ll pow){ // помощь для переовда строкив  число
    if (a == 1 || pow == 0) {
        return 1;
    } else if (pow == 1) {
        return a;
...
ll stringToInt(string a){// перевод стркои в число
    ll max_pow = a.size() - 1;
    ll res = 0;
...
inline void solve(){
    ...
    cin >> s;
    if(sz(s)==1){
        if(s[0]=='1' || s[0]=='0' || s[0]=='2' || s[0]=='3' || s[0]=='5' || s[0]=='7') cout << -1 << nl;
        return;
    }
    ll def_sum = 0;
    ll star = 0;
    ll number = stringToInt(s);
    fca(it, s){
        if(it!='*'){
            def_sum+=(it-'0');
        } else star++;
    }
    if(star==0 && easy_prime(number)) { cout << -1 << nl; return;}  
    string ans = s;
    ll lpos =  s.rfind('*');
    incr(i,0,sz(s)){
        if(s[i]=='*')
    ...
    //     ans_sum+=(it-'0');
    // }
    // if(ans_sum%3) cout << -1 << nl;
    ll sans = stringToInt(ans);
    if(easy_prime(sans)) cout << -1 << nl;
    else cout << ans << nl;
}
signed main(){
    IOS;
