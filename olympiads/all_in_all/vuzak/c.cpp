...
inline void solve(){
    int n;
    cin >> n;
    vc <pair<pair<ld,int>,pair<ld,ld>>> data(n);
    incr(i,0,n){
        cin >> data[i].second.first >> data[i].second.second;
        data[i].first.first = data[i].second.second/data[i].second.first;
        data[i].first.second = i+1;
    }
    sort(all(data));
    cout << 1<< nl;
    cout << data.front().first.second << nl;
    cout << 1<< nl;
    cout << data.back().first.second << nl;
}
signed main(){
    IOS;
    // prepare();
