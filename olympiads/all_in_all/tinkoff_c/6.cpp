...
pair <ll, ll> seg(ll x){
    pair <ll, ll> ans;
    ans.first = x - (x/m2)*m2;
    ans.second = (ans.first + duration)%m2;
    return ans;
}
inline void solve(){
    cin >> n >> h >> m >> duration;
    ...
    incr(i,0,n){
        ll a, b;
        cin >> a>> b;
        timetrains.push_back(id(a,b));
    }
    vector <vector<int>> segments;
    int c = 1;
    fca(it,timetrains){
        pair<ll,ll> sg = seg(it);
        vi t1,t2;
        t1.push_back(sg.first);
        t1.push_back(-1);
        t1.push_back(c);
        
        t2.push_back(sg.second);
        t2.push_back(1);
        t2.push_back(c);
    ...
    ll cur = 0;
    ll mx = 0;
    incr(i,0,segments.size()){
        cur-=segments[i][1];
        mx = min(mx,cur);
    }
    incr(i,0,segments.size()){
        if(cur==mx){
            cout << notcont.size() << nl;
            fca(it,notcont) cout << it << nl;
            return;
        } else {
            cur-=segments[i][1];
            mx = min(mx,cur);
            if(segments[i][1]==1) true;
        }
    }
}
signed main(){
