...
inline void solve(){
    ...
    while (n-->0){
        ...
        cin >> x;
        if(x==1) prog.push(c);
        else if (x==2) mat.push(c);
        else sport.push(c);
        c++;
    }
    cout << min({sz(prog), sz(mat), sz(sport)}) << nl;
    while (!prog.empty() && !mat.empty() && !sport.empty()){
        cout << prog.front() << " "  << mat.front() << " " << sport.front() << nl;
        prog.pop();
        mat.pop();
        sport.pop(); 
    }
