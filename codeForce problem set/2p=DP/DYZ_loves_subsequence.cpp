...
inline void solve(){
    int n;
    cin >> n;
    if(n<=2) {cout << n << nl; return;}
    n++;
    vi data(n), dp(n,0),dpr(n,0);
    incr(i,1,n){
        cin >> data[i];
    }
    dp[0] = 0;
    dpr[n] = 0;
    incr(i,1,n){
        if(data[i-1]<data[i]) dp[i] = dp[i-1] + 1;
        else dp[i] = 1;
    }
    decr(i,n-1,0){
        if(data[i+1]>data[i]) dpr[i] = dpr[i+1] + 1;
        else dpr[i] = 1;
    }
    int ans = - INF;
    incr(i,1,n-1){
    ...
    decr(i,n-1,1) ans = max(ans,dpr[i]+1);
    incr(i,1,n){
        if(data[i-1]<data[i+1]-1) ans = max(ans,dp[i-1]+dpr[i+1]+1);
    }
    cout << ans << nl;
}
signed main(){
